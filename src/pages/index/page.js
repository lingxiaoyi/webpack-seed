require('../../public-resource/sass/newindex.scss')
require('./style.scss')
let $ = require('../../../vendor/jquery.min')
import WebStorageCache from '../../../vendor/web-storage-cache.min'
...
/* let host = "http://172.18.250.87:8381/dfsports_h5/"//测试服
let hostDsp = "http://106.75.98.65/partner/list"//测试服 */
let host = 'http://dfsports_h5.dftoutiao.com/dfsports_h5/' // 线上
let hostDsp = 'http://dftyttd.dftoutiao.com/partner/list' //    线上
let homeLuoboApi = 'http://msports.eastday.com/json/msponts/home_lunbo.json'
let orderApi = 'http://dfty.dftoutiao.com/index.php/Home/WechatOrder/yuyue?system_id=9&machid='
...
module = (function(my) {
    ...
    my.GLOBAL = {// 工具函数
        ...
        getPageQid() {
            let qid = my.GLOBAL.getUrlParam('qid')
            let specialChannel
            if (qid) {
        ...
        getUid() {
            let uid = my.GLOBAL.CookieUtil.get('uid')
            if (!uid) {
                uid = (+new Date()) +
                    Math.random().toString(10).substring(2, 6)
                my.GLOBAL.CookieUtil.set('uid', uid,
        ...
        formatTimeToMatch(currentServerTime, startTime) {
            ...
            let todayTime = new Date(
                new Date(currentServerTime / 1).format('yyyy/MM/dd')).getTime()//今天12点的时间戳大小
            let chazhi = (new Date(timestamp).getTime() - todayTime) /
                (24 * 60 * 60 * 1000)
...
// 首页数据 index_new
module = (function(my) {
    const $swiperSlides = $('#mainSection').children(
        '.swiper-wrapper').children('.swiper-slide') //外层swiper
    //导航条元素
    let $body = $('body')
    let $header = $('header')
    let $headNav = $('#headNav')
    ...
    let $line = $headNav.find('.nav-new .line')
    let $pullDownLoadTips = $headNav.children('.pull-down-load-tips')
    let wsCache = new WebStorageCache()
    $body.append(`<div class="popup" id="popup"></div>`)//加入弹窗
    let $popup = $('#popup')
    //定义需要传入接口的值
    let os = my.GLOBAL.getOsType()
    let recgid = my.GLOBAL.getUid()
    let qid = my.GLOBAL.getPageQid()
    ...
    class EastSport {
        ...
        changeNavPos() {
            ...
            $headNavLi.click(function() {
                ...
                if (!that.firstIntoPage) {
                    wsCache.set(`scrollTop${that.curPos}`,
                        $swiperSlides.eq(that.curPos).scrollTop(),
                        {exp: 10 * 60})
                }

                $swiperSlides.eq(that.curPos).html(
                        `<div class="loading-tips1 l-btn-refresh1"><div><i class="iloading rotating"> </i>正在加载</div></div>`)
                $swiperSlides.hide()
                ...
                if (that.firstIntoPage) {
                    $line.css({
                        'left': `${my.GLOBAL.computedWidth(liWArr, i) + chazhi +
                        'px'}`,
                        'background': '#f62929',
                    })
                } else {
                ...
                //加载缓存的数据或者线上的
                if (wsCache.get(`newsModule${that.curPos}`) &&
                    wsCache.get(`newsModule${that.curPos}`).indexOf('iframe') >=
                    0 && wsCache.get(`newsModule${that.curPos}`).indexOf('data-dsp="hasDsp"') < 0) {
        ...
        loadFocusPic() {
            let that = this
            let $el = $(
                '<div id="swiperContainer" class="swiper-container fs-swiper-container"></div>')
            that.focusRequest = $.ajax({
                ...
                $el.append(
                    `<div class="swiper-wrapper">${produceHtml(result)}</div>`)
                $swiperSlides.eq(that.curPos).html('')
                ...
                data.forEach(function(item, i) {
                    if (i < 5) {
                        html += `<div class="swiper-slide">
                                     ...
                                     <p>${item.title}<span>${i + 1}&nbsp/&nbsp<i>5</i></span></p></a>
                                 </div>`
                    }
                })
                return html
        ...
        loadHotMatch() {
            ...
            let that = this
            that.indexmatchRequest = my.GLOBAL.makeJsonp(host + 'indexmatch',
                data).done(function(result) {
                if (result.data.length) { //有推荐赛程就添加元素
                    $el.append(
                        `<ul class="clearfix">${produceHtml(result)}</ul>`)
                    if (that.curPos !== 0) {
                        $swiperSlides.eq(that.curPos).html('')
                    }
                    $swiperSlides.eq(that.curPos).append($el)
            ...
            $body.on('click', '.btn-order', function() {
                ...
                isRequested = false
                my.GLOBAL.makeJsonp(orderApi + $(this).attr('data-matchid'),
                    {}).done(function(result) {
                    if (result.status === -1) {
                        popup(1)
                    } else {
            ...
            function produceHtml(result) {
                ...
                data.forEach(function(item, i) {
                    ...
                    if (i < 2) {
                        item.home_logoname = item.home_logoname ||
                            'http://msports.eastday.com/h5/img/logo_default.png'
                        item.visit_logoname = item.visit_logoname ||
                            'http://msports.eastday.com/h5/img/logo_default.png'
                        html += `<li class="clearfix">
                                ...
                                <div class="info" id="${item.matchid}">
                                    <div class="score">${item.ismatched === '-1'
                            ? my.GLOBAL.formatTimeToMatch(
                                item.currentServerTime, item.starttime)
                            : score}</div>
                                    ${orderStr}
                                </div>
                                <div class="team">
        ...
        initNews() {
            ...
            that.newspoolRequest = my.GLOBAL.makeJsonp(host + 'newspool', data).done(function(result) {
                    ...
                    $swiperSlides.eq(that.curPos).append($el)
                    let loading = $(
                        `<div class="loading-tips2 btn-refresh2"><span class="loading rotating"></span>正在载入新内容...</div>`)
                    ...
                    wsCache.set('startkeyArr', that.startkeyArr,
                        {exp: 10 * 60})
                    wsCache.set('newkeyArr', that.newkeyArr, {exp: 10 * 60})
                    wsCache.set('loadingHeight', that.loadingHeight,
                        {exp: 10 * 60})//保存load高度
                    that.delLoadingTips()
                    let i = that.curPos
                    setTimeout(function() {
                        wsCache.set(`newsModule${i}`,
                            $swiperSlides.eq(i).html(), {exp: 10 * 60})
                    }, 400)
                    that.pullUpLoadNews() //注册加载上拉更多新闻
                    that.pullDownLoadNews() //注册加载下拉更多新闻
                    that.btnLoadMoreNews() //注册点击下拉加载事件btn-load-more元素
                    ...
                    that.requestDspUrl().done(function(data) {
                        ...
                        for (let i = 0; i < 4; i++) {
                            if (that.dspData[i]) {
                                ...
                                let value = $(
                                    `#module${that.curPos + '_' +
                                    i}`).children().attr('id')
                                my.GLOBAL.getScript(
                                    `http://tt123.eastday.com/${value}.js`,
                                    function() {
                                    }, $('#' + value)[0])
                            }
                        }
        ...
        pullUpLoadNews() {
            ...
            $swiperSlides.eq(that.curPos).scroll(function() {
                ...
                //加5是为了减少误差
                if (slideHeight - that.loadingHeight + 5 >= tipsTop &&
                    that.pullUpFinished[that.curPos]) {
        ...
        loadNewsList() {
            ...
            my.GLOBAL.makeJsonp(host + 'newspool', data).done(function(result) {
                ...
                let i = that.curPos
                setTimeout(function() {
                    wsCache.set(`newsModule${i}`, $swiperSlides.eq(i).html(),
                        {exp: 10 * 60})
                }, 400)
            }).done(function() {
                that.requestDspUrl(3).done(function(data) {
                    ...
                    for (let i = 2; i >= 0; i--) {
                        if (that.dspData[i]) {
                            $(`#module${that.curPos + '_' +
                            (that.ggIndexArr[that.curPos] - 3 + i)}`).html(that.loadDspHtml(i))
                        } else {
                            let value = $(`#module${that.curPos + '_' +
                            (that.ggIndexArr[that.curPos] - 3 + i)}`).children().attr('id')
                            my.GLOBAL.getScript(
                                `http://tt123.eastday.com/${value}.js`,
                                function() {
                                }, $('#' + value)[0])
        ...
        produceListHtml(result, ggPos) {
            ...
            data.forEach(function(item, i) {
                ...
                if (that.curPos !== 0 && i === 4 && !mlinksLength) {
                    html += `<li class="clearfix" style="padding:0border: 0">
                            <div class="clearfix m-links">
                                ...
                                <div>
                                    <a href="data_all.html?datatype=p&classtype=${that.dataType.toLowerCase()}">
                                        <img src="${that.dataType === 'NBA'
                        ? 'http://msports.eastday.com/h5/img/i-paiming.png'
                        : 'http://msports.eastday.com/h5/img/i-jifenbang.png'}" alt=""/>
                                        <p>${that.dataType === 'NBA'
                        ? '排名'
                        : '积分榜'}</p>
                                    </a>
        ...
        pullDownLoadNews() {
            ...
            $swiperSlides.eq(that.curPos).on('touchend', function(e) {
                ...
                } else if (that.direction === 'slideUp' &&
                    that.isSlideMove === false) {
                    $header.hide()
                    $swiperSlides.each(function() {
                        $(this).css({
                            'height': that.slideHeight + that.headerHeight +
                            'px',
                        })
                    })
                }
                if (that.isTouchBottom && that.pullDownFinished) {
                    ...
                    if (that.pullDownFinished === true &&
                        that.isSlideMove === false) {
                        if ($pullDownLoadTips.attr('style').indexOf('slideDown') >= 0) {
                            that.setPullDownLoadTipsAnimation(
        ...
        loadgg() {
            let _newsGg_ = _indexGg_[currChannel].concat(_newsList_)
            let that = this
            let html = ''
            html += `<li style="padding:0" class="gg clearfix bdgg-wrap" data-ggid="${_newsGg_[that.ggIndexArr[that.curPos]]}" id="module${that.curPos +
            '_' + that.ggIndexArr[that.curPos]}">
        ...
        loadDspHtml(posi) {
            ...
            $body.append(
                `<img style="display: none" src="${item.showbackurl}"/>`)
            return html
        }
                ...
                offsetArr.forEach((item, index) => {
                    if (cHeight + $(this).scrollTop() > item) {
                        if (eleArr[index].attr('inviewbackurl')) {
                            $body.append(
                                `<img style="display: none" src="${eleArr[index].attr(
                                    'inviewbackurl')}"/>`)
        ...
        unload() {
            ...
            $(window).unload(function() {
                wsCache.set(`scrollTop${that.curPos}`,
                    $swiperSlides.eq(that.curPos).scrollTop(), {exp: 10 * 60})
            })
        }
